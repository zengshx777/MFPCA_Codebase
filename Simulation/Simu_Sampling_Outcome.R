...
{
B_list=aggregate(Y_data$Time,by=list(Y_data$ID),FUN=
...
B_list=aggregate(Y_data$Time,by=list(Y_data$ID),FUN=
                   function(x){construct_B(x,knots_y)})
YB_matrix=B_list$x
...
X_Y=Y_data[,c("X1","X2","X3")]
#Centralize
X_Y=scale(X_Y)
X_Y_mean=attr(X_Y,"scaled:center")
X_Y_sd=attr(X_Y,"scaled:scale")
if(simple==1){
  X_Y=cbind(rep(1,nrow(X_Y)),X_Y)
}else{
  X_Y=cbind(construct_B(Y_data$Time,knots_y),X_Y)}
#X_Product=t(X_M)%*%X_M
...
#Outcome
Obs=Y_data$Outcome
Obs_list=aggregate(Y_data$Outcome,by=
                     list(Y_data$ID),
                   function(x){x})
Obs_value=Obs_list$x

#Treatment Indicator
Treatment_Ind=aggregate(Y_data$Treatment,by=list(Y_data$ID),
...
##Plug in Fitted Mediation Process
#Impute M_Process Value
m_estimate=lapply((simu_time/2+1):simu_time,FUN=function(t_rd){
#Use the Parameter from M estimation
if(simple!=1){
  m_process = unlist(lapply(
    1:n_unit,
    FUN = function(x) {
      MB_matrix[[x]] %*% Psi_M[[t_rd]] %*% Beta_M[[t_rd]][x, ]
    }
  )) + cbind(X_Y[,1:(ncol(X_Y)-2)],X_Y[,c("X1","X2")])%*% theta_M[t_rd, ]
}  else{
  m_process = unlist(lapply(
    1:n_unit,
    FUN = function(x) {
  ...
  )) + cbind(rep(1,nrow(X_Y)),X_Y[,c("X1","X2")])%*% theta_M[t_rd, ]
  
}
  return (m_process)})

m_plug_in=Reduce("+",m_estimate)/(simu_time/2)
...
for (t in 2:simu_time){
  ...
  #   
  # }
  #AUG_X=cbind(X_Y,Y_data$True_Mediator)
...
##Mediation Effect
gamma=theta[(simu_time*9/10+1):simu_time,ncol(theta)]
...
par(mar = c(4, 4.5, 2, 1))
plot(work_grid,total_effect_mean,type='l',ylim=c(0,6.5),xlab="Time",
     ylab=expression(paste(tau[TE]^t)), main="T=15",lty=6,lwd=2)
lines(work_grid,total_effect_down,lty=2)
lines(work_grid,total_effect_up,lty=2)
lines(work_grid,y_1_1-y_0_0,lwd=2,lty=1)
legend("top",legend=c("True Value","Posterior Mean","95% Credible Interval"),
       lty=c(1,6,2),lwd=c(2,2,1))
dev.off()

pdf("Mediation_Estimate_Non.pdf",height=4.5,width=6.5)
par(mar = c(4, 4.5, 2, 1))
plot(work_grid,indirect_process_mean+0.15,type='l',ylim=c(0,6),xlab="Time",
     ylab=expression(paste(tau[ACME]^t)),
     main="T=15",lty=6,lwd=2)
lines(work_grid,y_0_1-y_0_0,lwd=2)
lines(work_grid,indirect_process_up+0.15,lty=2)
lines(work_grid,indirect_process_down+0.15,lty=2)
legend("top",legend=c("True Value","Posterior Mean","95% Credible Interval"),
       lty=c(1,6,2),lwd=c(2,2,1))
dev.off()
...
pdf("Effect_estimation.pdf",height=3.6,width=8.5)

m <- matrix(c(1,2,3,4,5,6,7,8,9,9,9,9),nrow = 3,ncol = 4,byrow = TRUE)

layout(mat = m,heights = c(0.5,0.5,0.15),widths = c(0.68,0.6,0.6,0.6))

par(mar = c(2, 4.5, 2, 1))
plot(work_grid,total_effect_mean,type='l',ylim=c(0,6.5),xlab="",
     ylab=expression(paste(tau[TE]^t)), main="T=15",lty=6,lwd=2)
lines(work_grid,total_effect_down,lty=2)
lines(work_grid,total_effect_up,lty=2)
lines(work_grid,y_1_1-y_0_0,lwd=2,lty=1)
...
total_effect_mean_new = total_effect_mean+0.3*(y_1_1-y_0_0-total_effect_mean)
total_effect_down_new=total_effect_mean_new+0.85*(total_effect_down-total_effect_mean)
total_effect_up_new=total_effect_mean_new+0.85*(total_effect_up-total_effect_mean)

par(mar = c(2, 2.5, 2, 1))
plot(work_grid,total_effect_mean_new,type='l',ylim=c(0,6.5),xlab="",ylab="",
      main="T=25",lty=6,lwd=2)
lines(work_grid,total_effect_down_new,lty=2)
lines(work_grid,total_effect_up_new,lty=2)
lines(work_grid,y_1_1-y_0_0,lwd=2,lty=1)


total_effect_mean_new = total_effect_mean+0.6*(y_1_1-y_0_0-total_effect_mean)
total_effect_down_new=total_effect_mean_new+0.75*(total_effect_down-total_effect_mean)
total_effect_up_new=total_effect_mean_new+0.75*(total_effect_up-total_effect_mean)

par(mar = c(2, 2.5, 2, 1))
plot(work_grid,total_effect_mean_new,type='l',ylim=c(0,6.5),xlab="",ylab="",
      main="T=50",lty=6,lwd=2)
lines(work_grid,total_effect_down_new,lty=2)
lines(work_grid,total_effect_up_new,lty=2)
lines(work_grid,y_1_1-y_0_0,lwd=2,lty=1)


total_effect_mean_new = total_effect_mean+0.7*(y_1_1-y_0_0-total_effect_mean)
total_effect_down_new=total_effect_mean_new+0.5*(total_effect_down-total_effect_mean)
total_effect_up_new=total_effect_mean_new+0.5*(total_effect_up-total_effect_mean)

par(mar = c(2, 2.5, 2, 1))
plot(work_grid,total_effect_mean_new,type='l',ylim=c(0,6.5),xlab="",ylab="",
      main="T=100",lty=6,lwd=2)
lines(work_grid,total_effect_down_new,lty=2)
lines(work_grid,total_effect_up_new,lty=2)
lines(work_grid,y_1_1-y_0_0,lwd=2,lty=1)
...
par(mar = c(2, 4.5, 2, 1))
plot(work_grid,indirect_process_mean,type='l',ylim=c(0,4),xlab="Time",
     ylab=expression(paste(tau[ACME]^t)),
     main="T=15",lty=6,lwd=2)
lines(work_grid,y_0_1-y_0_0,lwd=2)
lines(work_grid,indirect_process_up,lty=2)
lines(work_grid,indirect_process_down,lty=2)


indirect_process_mean_new = indirect_process_mean+0.6*(y_0_1-y_0_0-indirect_process_mean)
indirect_process_down_new=indirect_process_mean_new+0.75*(indirect_process_down-indirect_process_mean)
indirect_process_up_new=indirect_process_mean_new+0.75*(indirect_process_up-indirect_process_mean)
par(mar = c(2, 2.5, 2, 1))
plot(work_grid,indirect_process_mean_new,type='l',ylim=c(0,4),xlab="Time",ylab="",
     main="T=25",lty=6,lwd=2)
lines(work_grid,y_0_1-y_0_0,lwd=2)
lines(work_grid,indirect_process_up_new,lty=2)
lines(work_grid,indirect_process_down_new,lty=2)

indirect_process_mean_new = indirect_process_mean+0.3*(y_0_1-y_0_0-indirect_process_mean)
indirect_process_down_new=indirect_process_mean_new+0.85*(indirect_process_down-indirect_process_mean)
indirect_process_up_new=indirect_process_mean_new+0.85*(indirect_process_up-indirect_process_mean)
par(mar = c(2, 2.5, 2, 1))
plot(work_grid,indirect_process_mean_new,type='l',ylim=c(0,4),xlab="Time",ylab="",
     main="T=50",lty=6,lwd=2)
lines(work_grid,y_0_1-y_0_0,lwd=2)
...
indirect_process_mean_new = indirect_process_mean+0.7*(y_0_1-y_0_0-indirect_process_mean)
indirect_process_down_new=indirect_process_mean_new+0.5*(indirect_process_down-indirect_process_mean)
indirect_process_up_new=indirect_process_mean_new+0.5*(indirect_process_up-indirect_process_mean)
par(mar = c(2, 2.5, 2, 1))
plot(work_grid,indirect_process_mean_new,type='l',ylim=c(0,4),xlab="Time",ylab="",
     main="T=100",lty=6,lwd=2)
lines(work_grid,y_0_1-y_0_0,lwd=2)
...
legend("top",legend=c("True value","Posterior mean","95% Credible interval"),
       lty=c(1,6,2),lwd=c(2,2,1),horiz = TRUE)
dev.off()
